...
router.post('/movieGeneration', movieAuth, async (req, res) => {
    try {
        ...
        logger.error(`Error getting movies: ${err.message}`);
...
router.post('/returnMovies', movieAuth, async (req, res) => {
    try {
        ...
        res.status(500).send('Error retrieving user movies');
    }
});
...
router.post('/getPlaylists', movieAuth, async (req, res) => {
    try {
        ...
        res.status(500).send('Failed to get user playlists from database');
    }
});
